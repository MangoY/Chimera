...
import sys
from transformations import transform_and_extract
import pandas as pd
import numpy as np
import random
import re
...
class RandomDirectiveGenerator(DirectiveGenerator):
    def gen_single_dir(self, knob, context=None, clear_partition_scheme=False):
        ...
        if (knob.type == 'loop'): # if the type of the tunable knob (directive) is a loop
            ...
            parameters_dict['array_'+knob['name']+'_dim'+str(int(dim))+'_type'] = partition_scheme
            if partition_scheme != 'none':
                ...
                parameters_dict['array_'+knob['name']+'_dim'+str(int(dim))+'_factor'] = factor
            else: # if it is none, i.e. do not partition the array
                parameters_dict['array_'+knob['name']+'_dim'+str(int(dim))+'_factor'] = 0 # set the factor to 0 for safety
        else:
...
class DirectiveCrossover(DirectiveGenerator):
    ...
    def gen_single_dir(self, knob, context=None):
        ...
        if (knob.type == 'loop'): # if the type of the tunable knob (directive) is a loop
            ...
            if(variable not in self.partition_schemes.keys()):
                partition_scheme = parent['array_'+knob['name']+'_dim'+str(int(dim))+'_type']
                if (partition_scheme != 'none'): # if it's none then don't add the scheme to the dict
                    self.partition_schemes[variable] = partition_scheme
            else:
                # if other dimensions has choose the scheme, but parent is none, we can still choose the parent
                if (parent['array_'+knob['name']+'_dim'+str(int(dim))+'_type'] == 'none'):
                    partition_scheme = parent['array_'+knob['name']+'_dim'+str(int(dim))+'_type']
            ...
            if partition_scheme != 'none':
                ...
                if partition_scheme != 'complete':
                    ...
                    factor = parent['array_'+knob['name']+'_dim'+str(int(dim))+'_factor']
...
class DirectiveMutator(DirectiveGenerator):
    ...
    def gen_single_dir(self, knob, context=None):
        ...
        if (knob.type == 'loop'): # if the type of the tunable knob (directive) is a loop
            ...
            importance_type = importances['array_'+knob['name']+'_dim'+str(int(dim))+'_type']
            importance_factor = importances['array_'+knob['name']+'_dim'+str(int(dim))+'_factor']

            # to ensure that the partitioning scheme is consistent for an array
            # check if there is already a partitioning scheme picked for this array
            # if so, just use the same factor
            if (rand_val_type < importance_type):
                selected_scheme = random_parameters['array_'+knob['name']+'_dim'+str(int(dim))+'_type']
            else:
                selected_scheme = offspring['array_'+knob['name']+'_dim'+str(int(dim))+'_type']
            ...
            if partition_scheme != 'none':
                ...
                if partition_scheme != 'complete':
                    # get the factor from input parameters
                    if (rand_val_factor < importance_factor):
                        factor = random_parameters['array_'+knob['name']+'_dim'+str(int(dim))+'_factor']
                    else:
                        factor = offspring['array_'+knob['name']+'_dim'+str(int(dim))+'_factor']
                ...
                parameters_dict['array_'+knob['name']+'_dim'+str(int(dim))+'_factor'] = 0
        else:
            raise AssertionError('Invalid knob type')
...
class DirectiveWriter():
    ...
    def gen_single_dir(self, knob, context):
        ...
        if (knob.type == 'loop'): # if the type of the tunable knob (directive) is a loop
            ...
            dim = int(knob['dim'])
            variable = '{0} {1}'.format(knob['scope'], knob['name'])
            factor = context['array_'+knob['name']+'_dim'+str(int(dim))+'_factor']
            
            selected_scheme = context['array_'+knob['name']+'_dim'+str(int(dim))+'_type']
            if(variable not in self.partition_schemes.keys()):
                partition_scheme = selected_scheme
                if (partition_scheme != 'none'):
                    self.partition_schemes[variable] = partition_scheme
            ...
            if partition_scheme != 'none':
                ...
                parameters_dict['array_'+knob['name']+'_dim'+str(int(dim))+'_factor'] = 0 # set the factor to 0 
        else:
            raise AssertionError('Invalid knob type')
